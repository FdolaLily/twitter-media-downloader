...
async function plan(medias) {
    const tasks = [], statuses = [], history = [];
    ...
    app.ui = { setButtonStatus: (btn, status) => statuses.push(status), updateHistoryCount() {}, lang: { completed: 'Done' } };
    app.queue = { add: task => tasks.push(task) };
    await app.handleDownloadClick({ classList: { contains: () => false }, dataset: {} }, '123', false);
    return { tasks, statuses, history };
}
...
(async () => {
    ...
    await check('download retries encode once and report success only once', async () => {
        ...
        GifConverter.convert = async () => { conversions++; return originalGif; };
        sandbox.GM_download = o => { downloadUrl = o.url; attempts++; if (attempts < 3) o.onerror(new Error('temporary')); else o.onload(); };
        await new DownloadQueue().start({ gif: true, url: 'source.mp4', name: 'output.gif', onload: n => { success++; assert.equal(n, originalGif.size); }, onerror: () => failed++ });
